...
/**
 * Esta es una versión modificada del Tilesheet que viene con NME para omitir este bug:
 * http://www.flash.io/community/forums/bugs/problem-vector-and-tilesheet/
 * 
 */
extern class Tilesheet {
	 ...
	 * 
	 * @param	graphics		The flash.display.Graphics object to use for drawing
...
class Tilesheet
{
	...
	public function drawTiles(graphics:Graphics, tileData:Array<Float>, smooth:Bool = false, flags:Int = 0):Void
	{
		...
		else
		{
			...
			{
				var x:Float = tileData[index];
				var y:Float = tileData[index + 1];
				var tileID:Int = Math.floor(tileData[index + 2]);
				index += 3;
				
				var tile:Rectangle = tiles[tileID];
				var centerPoint:Point = tilePoints[tileID];
				var ox = centerPoint.x * tile.width;
				var oy = centerPoint.y * tile.height;
