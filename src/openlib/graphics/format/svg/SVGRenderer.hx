...
import openlib.graphics.format.svg.PathParser;
import openlib.graphics.format.svg.PathSegment;
...
import openlib.graphics.format.svg.Grad;
import openlib.graphics.format.svg.Group;
import openlib.graphics.format.svg.FillType;
import openlib.graphics.format.gfx.Gfx;
import flash.geom.Rectangle;
...
class SVGRenderer
{
    ...
    {
       return new SVGRenderer(new SVGData(inXML,true)).iterate(new openlib.graphics.format.gfx.Gfx2Haxe(),inFilter).commands;
    }

    public static function toBytes(inXML:Xml,?inFilter:ObjectFilter) : openlib.graphics.format.gfx.GfxBytes
    {
       return new SVGRenderer(new SVGData(inXML,true)).iterate(new openlib.graphics.format.gfx.GfxBytes(),inFilter);
    }
    ...
    public function iteratePath(inPath:Path)
    {
       ...
       if (!geomOnly)
       {
          ...
          {
             var style = new openlib.graphics.format.gfx.LineStyle();
    ...
    public function render(inGfx:Graphics,?inMatrix:Matrix, ?inFilter:ObjectFilter, ?inScaleRect:Rectangle,?inScaleW:Float, ?inScaleH:Float )
    {
       ...
       mGfx = new openlib.graphics.format.gfx.GfxGraphics(inGfx);
       if (inMatrix==null)
          mMatrix = new Matrix();
       else
        ...
        Rectangle
    {
       ...
       var gfx = new openlib.graphics.format.gfx.GfxExtent();
       mGfx = gfx;
       if (inMatrix==null)
          mMatrix = new Matrix();
       else
    ...
    public function findText(?inFilter:ObjectFilter)
    {
       mFilter = inFilter;
       mGroupPath = [];
       var finder = new openlib.graphics.format.gfx.GfxTextFinder();
       mGfx = finder;
    ...
    public function renderBitmap(?inRect:Rectangle,inScale:Float = 1.0)
    {
       ...
       mGfx = new openlib.graphics.format.gfx.GfxGraphics(shape.graphics);
